...
- **통계 밴드**(bull/bear): `backtest.py` 워크포워드로 검증. 변동성은 **GARCH(1,1)
  h개월 누적**(단계 F, √h 대체), 분위계수는 **split-conformal 실측 잔차분위**
  (단계 C, `calibration.json` — 팩테일·상하방 비대칭). 백테스트상 나이브 대비
  스킬이 통계적으로 유의(DM p=0.01). 중심선(base)은 랜덤워크에 가깝게 유지(단계 D
  에서 통계 앙상블이 나이브를 못 이김을 확인).
